...
def _send_email(to_email, subject, body):
    try:
        ...
        print(f"[SUCCESS] Email sent -> {to_email}")
    except Exception as e:
        print(f"[ERROR] Email error ({to_email}): {e}")
...
def _save_to_firebase(data):
    try:
        ...
        print(f"[SUCCESS] Saved to Firebase -> {new_contact.key}")
    except Exception as e:
...
def _process_contact(data):
    ...
    futures = [
        ...
        _executor.submit(
            _send_email,
            ADMIN_EMAIL or SMTP_EMAIL,  # Fallback to SMTP_EMAIL if ADMIN_EMAIL is not configured
